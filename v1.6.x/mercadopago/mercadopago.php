...
class MercadoPago extends PaymentModule
{
    ...
    public function __construct()
    {
        ...
        if (Language::getIsoById(Configuration::get('PS_LANG_DEFAULT')) == 'es') {
            $this->description = $this->l(
                'Mercado Pago Module Oficial - Recibir sus pagos con tarjeta de crédito, ticket y dinero en cuenta de Mercado Pago.'
            );
            $this->confirmUninstall = $this->l('¿Está seguro de que desea desinstalar MercadoPago?');
            $this->textshowemail = $this->l('Usted debe seguir las reglas de mercado para la compra a ser válida');
        } elseif (Language::getIsoById(Configuration::get('PS_LANG_DEFAULT')) == 'br') {
            $this->description = $this->l(
                'Mercado Pago Module Ofical - Receba seus pagamentos através de cartão de crédito ou boletos. Pode utilizar o checkout personalizado ou básico com redirecionamento.'
            );
            $this->confirmUninstall = $this->l('Você tem certeza que deseja desinstalar?');
            $this->textshowemail = $this->l('Você precisa seguir as regras de pagamento do Mercado Pago para a compra ser valida');
        } else {
            $this->description = $this->l(
                'Mercado Pago Module Ofical - Receive your payments with Credit Card, Ticket and Money in account of Mercado Pago.'
            );
    ...
    private function createTables()
    {
        ...
        $sql = " CREATE TABLE IF NOT EXISTS " . _DB_PREFIX_ .  "mercadopago_orders (
    ...
    private function createTableBoleto()
    {
        $sql = " CREATE TABLE IF NOT EXISTS " . _DB_PREFIX_ .  "mercadopago_boleto (
              `mercadopago_id` int(11) unsigned NOT NULL auto_increment,
              `cpf` varchar(30),
              `email` varchar(50),
              `cart_id` int(15),
              `added` datetime,
        ...
        error_log("criação das tabelas de boleto");
        return true;
    }
    ...
    private function getInfomationsForTicket($id_address_invoice)
    {
        ...
        error_log("=== retorno do Select resultFieldsTicket === " . Tools::jsonEncode($resultFieldsTicket));
    ...
    public function getContent()
    {
        ...
        } elseif (Tools::getValue('viewPayments')) {

        } elseif (Tools::getValue('submitmercadopago')) {

            error_log("===MERCADOPAGO_CREDITCARD_ACTIVE====".Tools::getValue('MERCADOPAGO_CREDITCARD_ACTIVE'));

            error_log("===MERCADOPAGO_PAGOFACIL_ACTIVE====".Tools::getValue('MERCADOPAGO_PAGOFACIL_ACTIVE'));
                ...
                if (!$this->validateCredential($client_id, $client_secret)) {
                    ...
                    if (!$this->mercadopago->isTestUser()) {
                        ...
                        switch ($current_country) {
                            case 'MLB':
                                Configuration::updateValue('MERCADOPAGO_SPONSOR_ID', 178326379);
                                break;
                            case 'MLM':
                                Configuration::updateValue('MERCADOPAGO_SPONSOR_ID', 187899553);
                                break;
                            case 'MLA':
                                Configuration::updateValue('MERCADOPAGO_SPONSOR_ID', 187899872);
                                break;
                            case 'MCO':
                                Configuration::updateValue('MERCADOPAGO_SPONSOR_ID', 187900060);
                                break;
                            case 'MLV':
                                Configuration::updateValue('MERCADOPAGO_SPONSOR_ID', 187900246);
                                break;
                            case 'MLC':
                                Configuration::updateValue('MERCADOPAGO_SPONSOR_ID', 187900485);
                                break;
                            case 'MPE':
                                Configuration::updateValue('MERCADOPAGO_SPONSOR_ID', 217182014);
                                break;
                            case 'MLU':
                                Configuration::updateValue('MERCADOPAGO_SPONSOR_ID', 241730009);
                                break;
                        }
                    }
                }
        ...
        $offline_payment_settings = array();
        if (isset($offline_methods_payments)) {
            foreach ($offline_methods_payments as $offline_payment) {
                $op_banner_variable = 'MERCADOPAGO_'.Tools::strtoupper($offline_payment['id'].'_BANNER');
                $op_active_variable = 'MERCADOPAGO_'.Tools::strtoupper($offline_payment['id'].'_ACTIVE');
        ...
        $settings = array(
            ...
            'checkout_2' => htmlentities(Configuration::get('MERCADOPAGO_CHECKOUT_2'), ENT_COMPAT, 'UTF-8'),
    ...
    private function validateCredential($client_id, $client_secret)
    {
        ...
        error_log("====getAccessToken====".Tools::jsonEncode($mp->getAccessToken()));
    ...
    public function hookPaymentReturn($params)
    {
        ...
        if (Tools::getValue('payment_method_id') == 'bolbradesco' ||
            ...
            error_log("====vai retornar para o boleto====");
    ...
    private function getPrestashopPreferencesStandard()
    {
        ...
        foreach ($products as $key => $product) {
            ...
            $item = array(
                ...
                'description' => $product['description_short'],
                'quantity' => $product['quantity'],
                'unit_price' => $round ? round($product['price_wt']) : Tools::ps_round((float)$product['price_wt'], 2),
                'picture_url' => (Configuration::get('PS_SSL_ENABLED') ? 'https://' : 'http://').$imagePath,
                'category_id' => Configuration::get('MERCADOPAGO_CATEGORY'),
            );
            if ($key == 0) {
    ...
    public function listenIPN($checkout, $topic, $id)
    {
        ...
        if ($checkout == 'standard' && $topic == 'merchant_order' && $id > 0) {
            ...
            if (! isset($result['error'])) {
                ...
                error_log("===updateOrder jfkldjlasdjfklas djfklas checkout===");
                $this->updateOrder(
                    $payment_ids,
                    $payment_statuses,
                    $payment_method_ids,
    ...
    private function updateOrder(
        ...
        error_log("entrou aqui updateOrder");
        $order = null;
        ...
        if (count($payment_statuses) == 1 ||
            ...
            error_log(print_r($payment_ids, true));

            error_log("entrou aqui!!!! transaction_id ====" . implode(' / ', $payment_ids));
            ...
            error_log("VAI REALIZAR O UPDATE ====");
            $payments[0]->transaction_id = implode(' / ', $payment_ids);
            $payments[0]->update();
            error_log("UPDAT RELIZADO====");
        }
    }
