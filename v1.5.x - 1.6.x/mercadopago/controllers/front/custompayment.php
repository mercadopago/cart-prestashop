...
class MercadoPagoCustomPaymentModuleFrontController extends ModuleFrontController
{
    ...
    private function placeOrder()
    {
        ...
        if ($order_status != null) {
            ...
            $uri = __PS_BASE_URI__.'order-confirmation.php?id_cart='.$cart->id.'&id_module='.$mercadopago->id.'&id_order='.$mercadopago->currentOrder.'&key='.$order->secure_key.'&payment_id='.$response['id'].'&payment_status='.$response['status'];

            if (Tools::getIsset('card_token_id')) {
                //$this->salveCard($_POST, $cart);
                ...
                $uri .= '&card_token='.Tools::getValue('card_token_id').'&card_holder_name='.$cardholderName.'&four_digits='.$four_digits.'&payment_method_id='.$response['payment_method_id'].'&payment_type='.$response['payment_type_id'].'&installments='.$response['installments'].'&statement_descriptor='.$response['statement_descriptor'].'&status_detail='.$response['status_detail'].'&amount='.$response['transaction_details']['total_paid_amount'];
            } else {
                $uri .= '&payment_method_id='.$response['payment_method_id'].'&payment_type='.$response['payment_type_id'].'&boleto_url='.urlencode($response['transaction_details']['external_resource_url']);
            }
            ...
            if (array_key_exists('message', $response) && (strpos($response['message'], 'Invalid users involved') !== false || (strpos($response['message'], 'users from different countries') !== false))) {
                ...
                $data['amount'] = Tools::displayPrice($response['transaction_details']['total_paid_amount'], new Currency(Context::getContext()->cart->id_currency), false);
