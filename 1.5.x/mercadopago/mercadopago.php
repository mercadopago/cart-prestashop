...
include_once (dirname(__FILE__) . '/includes/mercadopago.php');
...
class MercadoPago extends PaymentModule
{
    ...
    public function __construct()
    {
        ...
        $this->description = $this->l(
            'Receive payments via MercadoPago of credit cards and tickets using our custom checkout or standard checkout'
        );
    ...
    public function getContent()
    {
        ...
        if (Tools::getValue('login')) {
            ...
            try {
                ...
                UtilMercadoPago::logMensagem('MercadoPago::getContent - Fatal Error: ' . $e->getMessage(), MP_SDK::FATAL_ERROR);
    ...
    public function hookDisplayHeader()
    {
        ...
        $this->context->controller->addCss(
            $this->_path . 'views/css/mercadopago_v' . UtilMercadoPago::getPrestashopVersion() . '.css', 
            'all');
    ...
    public function hookPayment($params)
    {
        ...
        if ($this->hasCredential()) {
            ...
            $data = array(
                ...
                'version' => UtilMercadoPago::getPrestashopVersion(),
                'custom_action_url' => $this->link->getModuleLink('mercadopago',
                    'custompayment',
                    array(),
            ...
            if (Configuration::get('MERCADOPAGO_STANDARD_ACTIVE') == 'true') {
                ...
                if (array_key_exists('init_point', $result['response'])) {
                    ...
                    UtilMercadoPago::logMensagem('MercadoPago::hookPayment - An error occurred during preferences creation.' .
                        'Please check your credentials and try again.: ', MP_SDK::FATAL_ERROR);
                }
            }
        ...
        if (Tools::getValue('payment_method_id') == 'bolbradesco' ||
            ...
            $this->context->smarty->assign(
                array(
                    ...
                    'payment_method_id' => Tools::getValue('payment_method_id'),
                    'installments' => Tools::getValue('installments'),
                    'transaction_amount' => Tools::displayPrice(Tools::getValue('amount'),
                        $params['currencyObj'],
                        false),
                    'statement_descriptor' => Tools::getValue('statement_descriptor'),
                    'payment_id' => Tools::getValue('payment_id'),
    ...
    private function getPreferencesCustom($post)
    {
        ...
        $notification_url = $this->link->getModuleLink('mercadopago',
            'notification',
            array(),
            Configuration::get('PS_SSL_ENABLED'),
            null,
        ...
        if ($mercadopago_coupon != "") {
            ...
            if ($coupon['status'] == 200) {
                ...
                UtilMercadoPago::logMensagem($coupon['response']['error'] . Tools::jsonEncode($coupon), MP_SDK::FATAL_ERROR);                
                $this->context->smarty->assign(
                    array(
    ...
    public function listenIPN($checkout, $topic, $id)
    {
        ...
        if (Configuration::get('MERCADOPAGO_LOG') == 'true') {
            UtilMercadoPago::logMensagem('MercadoPago :: listenIPN - topic = ' . $topic, MP_SDK::INFO);
            UtilMercadoPago::logMensagem('MercadoPago :: listenIPN - id = ' . $id, MP_SDK::INFO);
            UtilMercadoPago::logMensagem('MercadoPago :: listenIPN - checkout = ' . $checkout, MP_SDK::INFO);
        }
        ...
        if (count($payment_statuses) == 1 ||
            ...
            if ($order_status) {
                ...
                if ($id_order) {
                    ...
                    error_log("=======existStates===========".$existStates);
                    if ($existStates) {
                        return;
                    }
                }
